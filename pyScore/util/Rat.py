...
class Rat:
	...
	def __init__(self, num, den = 1):
		...
		try:
			...
			if self.num == numi and self.den == deni:
				self.num = numi
				self.den = deni
	...
	def __add__(a, b):
		try:
			return rat(a.num * b.den + b.num * a.den,
				   a.den * b.den)
		except OverflowError:
			return rat(long(a.num) * long(b.den) +
				   long(b.num) * long(a.den),
				   long(a.den) * long(b.den))
	...
	def __mul__(a, b):
		try:
			...
			return rat(long(a.num) * long(b.num),
				   long(a.den) * long(b.den))
	...
	def __pow__(a, b):
		if b.den != 1:
			if type(a.num) is ComplexType:
				a = complex(a)
			else:
				a = float(a)
			if type(b.num) is ComplexType:
				b = complex(b)
			else:
				b = float(b)
			return a ** b
		try:
			return rat(a.num ** b.num, a.den ** b.num)
		except OverflowError:
			return rat(long(a.num) ** b.num,
				   long(a.den) ** b.num)
	...
	def __neg__(a):
		try:
			...
			# a.num == sys.maxint
			return rat(-long(a.num), a.den)

	# abs(a)
	def __abs__(a):
		return rat(abs(a.num), a.den)
	...
	# complex(a)
	def __complex__(a):
		return complex(a.num) / complex(a.den)

	# cmp(a,b)
	def __cmp__(a, b):
		## I modified this: all comparisons are based on floats
		## which prevents the really heavy rational subtraction
		## operation.  Doesn't seem to cause precision problems
